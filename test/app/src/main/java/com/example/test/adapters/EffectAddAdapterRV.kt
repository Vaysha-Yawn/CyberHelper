...
class EffectAddAdapterRV():
    ...
    class TemplateHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(effect: EffectAdd, newOrPres: Boolean, groupTitle:String, readOrEdit:Boolean, indexItem:Int, indexEff:Int) = with(binding){
            if (readOrEdit){//read
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TemplateHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.card_effect_add, parent, false)
    ...
    fun setData(list: RealmList<EffectAdd>, groupTitle:String, newOrPres: Boolean, readOrEdit:Boolean, indexItem:Int){
